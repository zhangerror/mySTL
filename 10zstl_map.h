...
__MYSTL_NAMESPACE_BEGIN_
	...
	class map {
	public:
		typedef Key					key_type;		//键型别
		typedef T					data_type;		//数值型别
		typedef T					mapped_type;
		typedef pair<const Key, T>	value_type;		//元素型别(键/值)
		typedef Compare				key_compare;	//键比较函数

		//以下定义一个 functor，调用" 元素比较函数 "
	...
	private:
		//以下定义表述型别，以 map 元素型别(一个 pair )的第一型别，作为 RBTree 节点的键值型别
	...
	public:
		...
		//以下所有的 map 操作行为，RBTree 都已提供，map 只要转调即可
